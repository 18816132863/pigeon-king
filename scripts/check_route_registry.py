...
class RouteRegistryChecker:
    ...
    def _check_handlers(self, registry: Dict):
        ...
        for route_id, route in routes.items():
            ...
            handler_type = route.get("handler_type", "")
            ...
            # 虚拟 device capability handler 检查
            if handler_type == "virtual_device_capability":
                # 不做 Python import 检查
                # 只检查 resolved_by 和必要字段
                if resolved_by != "device_capability_bus.executor":
                    self.errors.append(
                        f"路由 {route_id} 是虚拟 handler 但 resolved_by 不是 device_capability_bus.executor"
                    )
                
                # 检查 device_capability_bus.executor 可以 import
                import sys
                ...
                if not route.get("route_id"):
                    self.errors.append(f"路由 {route_id} 缺少 route_id 字段")
                if not route.get("capability"):
                    self.errors.append(f"路由 {route_id} 缺少 capability 字段")
                if not route.get("risk_level"):
                    self.errors.append(f"路由 {route_id} 缺少 risk_level 字段")
                if not route.get("policy"):
                    self.errors.append(f"路由 {route_id} 缺少 policy 字段")
