...
def _load_context_layer(goal: str, persona_state: dict | None = None,
    ...
    if not _CONTEXT_MODULES_LOADED:
        result['proactive_skill_suggestions'] = _v105_skill_suggestions(message if 'message' in locals() else None)
    return result
    try:
        # capsule
...
def run(message=None, goal=None, mode="pre_reply", **kwargs):
    ...
    try:
        ...
        payload['proactive_skill_suggestions'] = _v105_skill_suggestions(message or goal if 'goal' in globals() else message)
    return payload
    except Exception as e:
        return {
            "status": "warning",
...
# V104_3_MAINLINE_RUNTIME_FUSION
try:
    _V104_3_PREVIOUS_RUN = run
except NameError:  # pragma: no cover
    _V104_3_PREVIOUS_RUN = None

def _v104_3_runtime_fusion_summary():
    try:
        from orchestration.runtime_bus import summarize
        bus = summarize()
    except Exception as e:
        bus = {"status": "warning", "error": str(e)}
    try:
        from infrastructure.skill_policy_gate import check_action
        skill_policy = check_action("mainline_hook", "pre_reply")
    except Exception as e:
        skill_policy = {"status": "warning", "error": str(e)}
    try:
        from governance.runtime_commit_barrier_bridge import check_action
        barrier = check_action("pre_reply_context_load", source="mainline_hook")
    except Exception as e:
        barrier = {"status": "warning", "error": str(e)}
    return {
        "runtime_bus_linked": bus.get("runtime_bus_linked", False),
        "skill_policy_linked": skill_policy.get("status") in ("ok", "blocked"),
        "commit_barrier_bridge_linked": barrier.get("status") in ("ok", "blocked"),
        "heavy_chain_triggered": False,
        "side_effects": False,
    }
...
def run(message=None, goal=None, mode="pre_reply", **kwargs):
    try:
        if _V104_3_PREVIOUS_RUN:
            payload = _V104_3_PREVIOUS_RUN(message=message, goal=goal, mode=mode, **kwargs)
        else:
            payload = {"status": "ok", "mode": mode}
        if hasattr(payload, "to_dict"):
            payload = payload.to_dict()
        if not isinstance(payload, dict):
            payload = {"status": "ok", "value": str(payload), "mode": mode}
        payload.setdefault("status", "ok")
        payload.setdefault("fail_soft", True)
        payload.setdefault("heavy_chain_triggered", False)
        payload["runtime_fusion_summary"] = _v104_3_runtime_fusion_summary()
        payload['proactive_skill_suggestions'] = _v105_skill_suggestions(message or goal if 'goal' in globals() else message)
    return payload
    except Exception as e:
        return {
            ...
            "error": str(e),
            "heavy_chain_triggered": False,
            "runtime_fusion_summary": {"runtime_bus_linked": False, "error": str(e)},
        }
