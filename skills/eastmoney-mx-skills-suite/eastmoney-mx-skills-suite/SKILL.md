...
打开东方财富妙想平台注册页面：
**https://marketing.dfcfs.com/views/finskillshub/indexo3rSI1vW**
...
```bash
# Linux / macOS
mkdir -p ~/.config/mx-skills-suite
echo "your_api_key" > ~/.config/mx-skills-suite/api_key
...
API_KEY="${MX_APIKEY:-$(cat ~/.config/mx-skills-suite/api_key 2>/dev/null)}"
if [ -z "$API_KEY" ]; then
  echo "缺少 API Key，请按 步骤4：配置环境变量 步骤配置"
  exit 1
fi

export MX_APIKEY=${API_KEY}
```

配置完成后，尝试调用任意子技能接口验证 Key 是否有效。如果返回 `114` 或 `116` 错误码，说明 Key 无效，需要重新获取。


**若环境变量未设置，优先查看api_key文件内容是否配置，若配置，将文件内容导出环境变量：**
```bash
export MX_APIKEY=`cat ~/.config/mx-skills-suite/api_key`
```
